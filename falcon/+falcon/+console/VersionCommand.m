classdef VersionCommand < falcon.console.Command
    % Displays the falcon toolbox version.
    %  
    % <Syntax>
    % ####{regexprep(evalc('falcon help version'), {'\s+$', '\n', '<.*?>'}, {'', ['\n    % '], ''})}####
    %  
    % <Keywords>
    % Command!Version;falcon
    ...
    methods
        function varargout = call(~, ~)
        % falcon.console.VersionCommand/call is a function.
        %   varargout = call(~, ~)
        end

        function obj = VersionCommand()
        % Displays the falcon toolbox version.
        %  
        % <Syntax>
        % ####{regexprep(evalc('falcon help version'), {'\s+$', '\n', '<.*?>'}, {'', ['\n    % '], ''})}####
