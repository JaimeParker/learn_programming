classdef SimpleQuadraticPathFunction < falcon.lib.SimpleFunction
    % -------------------------------------------------------------------------
    %                                FALCON.m
    ...
    properties
        % falcon.lib.SimpleQuadraticPathFunction/NumSamples is a property.
        NumSamples
        % falcon.lib.SimpleQuadraticPathFunction/VariablePermutation is a property.
        VariablePermutation
        % falcon.lib.SimpleQuadraticPathFunction/WeightMatrix is a property.
        WeightMatrix
        % falcon.lib.SimpleQuadraticPathFunction/Offset is a property.
        Offset
        % falcon.lib.SimpleQuadraticPathFunction/StateOffset is a property.
        StateOffset
        % falcon.lib.SimpleQuadraticPathFunction/ControlOffset is a property.
        ControlOffset
        % falcon.lib.SimpleQuadraticPathFunction/ParameterOffset is a property.
        ParameterOffset
        % falcon.lib.SimpleQuadraticPathFunction/OutputOffset is a property.
        OutputOffset
        % falcon.lib.SimpleQuadraticPathFunction/UsePagemtimes is a property.
        UsePagemtimes
    ...
    methods
        ...
        %  
        % <Description>
        % Set the weight matrix and offset coefficients, preserving the
        % number of samples.
        end

        function self = SimpleQuadraticPathFunction(variables, varargin)
        % Create a quadratic path function.
        %  
        % <Syntax>
        % [ f ] = falcon.lib.SimpleQuadraticPathFunction(variables)
        % [ f ] = falcon.lib.SimpleQuadraticPathFunction(..., 'Name', Value)
        %  
        % <Description>
        % Create a quadratic path function (objective/constraint) of the
        % form
        %   value[i] = 0.5 * (v[i] - v0[i])' * W[i] * (v[i] - v0[i])
