classdef LagrangeFunction < falcon.core.PathFunction
    % Container for Lagrange cost functions
    ...
    properties
        % The original parameters and the initial and the final time influencing the Hessian calculation
        ParamHess
        % The indices in the array ParamHess
        RelevantParamHess
        % start index for time of Lagrange cost function
        startIdxRealTime
        % end index for time of Lagrange cost function
        endIdxRealTime
        % the normalized time grid step for the integration
        dtau_2_comp
    ...
    methods
        ...
        %  
        % <Syntax>
        % obj.addConstants(Constant1, Constant2, ..)
        %  
        % <Description>
        ...
        %  
        % <Inputs>
        % > Constant1: An array of constant numbers used in this
        % PathFunctionfor the first constant.
        % > Constant2: An array of constant numbers used in this
        % PathFunctionfor the second constant.
        % > INFO: Note that the third matrix dimension for constants
        % with MultipleTimeEvaluation set to true contains the
        % evolution over the time steps, i.e., constants of size [n,m]
        % for each discretization step in the phase.
        %  
        % <Keywords>
